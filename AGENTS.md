...
- A document is writable exactly when it is readable as text: the write tools take markdown descriptions and text originals of any format (regenerating the projection through the upload pipeline) and reject binaries, which are replaced by uploading.
  ...
  A whole write to a delimited suffix is checked against its own header by `tools.mutations.check_delimited_rows`, since a table built a row at a time is one f-string away from a row of the wrong width, and every value on that row then lands under the wrong heading with nothing downstream to report it.
  The header decides the width and `converters.DELIMITERS` decides the separator, one table read by both the write gate and `query_table`'s loader, so a `.csv` is comma-separated by the name it was given and the surface that writes one can never disagree with the surface that reads it back; an edit replaces a string inside a file it did not build and is left alone.
...
- `query_table` takes one path or a list, registering them as `t`, `t2`, `t3` so a join needs only the SQL, and the names are positional because an exported workbook's filename is a sentence with spaces that every query would have to quote.
  The dialect is named as Polars SQL wherever it is described, and an `unsupported function` refusal says so, since a run that reads it as "SQL cannot do this" leaves for Python over a spelling: the whole motivating task is one query, `QUANTILE_CONT` and all. `SHOW TABLES` works; `SHOW FUNCTIONS` is parsed but unimplemented upstream, so no function list is enumerable and none is vendored into a docstring.
...
- `query_table` reports what is wrong with the data and repairs none of it, since every repair is a decision about someone else's measurements.
  A mixed column names the values it could not parse, not only the share that did, and that one report is the whole of it: a repeated label is a header row spilled into the data, a `<100` is a reading past a measurement limit, a `0,05` is a decimal comma, and naming the value covers every shape where a rule per shape covers one.
  The report rides every call and is scoped to the columns the query named (carried in the result or spelled in the SQL, since `SELECT *` spells none), the cap leads with the worst shortfall and says what it cut, and a column nothing parses in is text rather than a defect and stays unreported.
- A tool that is both registered and injected is called inside `run_python` whenever a later step of the same program uses its result, and `output_path` is for a result a later *tool call* consumes.
  The two used to claim the same trigger in the same words, which bought a redirect-to-file-then-read-it-back detour for every spreadsheet question.
...
- Every bulk-output tool declares an `output_path` that stores the result in the workspace and returns a receipt, and that write answers to the same approval gate as the write tools.
  A display budget never cuts the stored result, only what is printed, and a receipt repeats any cut its payload does carry (`RedirectedOutput.truncated`): a file written under a receipt that reads like the whole of it is a wrong answer nothing downstream can catch.
  Only what the caller asked for bounds the data — `row_limit`, `max_results`, `max_chars` — while `max_formatted_chars` binds the text and says in a hint where it stopped (`query_table`, `read_document`; `jq`, `grep`, `search`, and the listings already worked this way).
...
- `run_python` is a Monty sandbox with no network or host filesystem: the workspace is a read-only mount (`tools/workspace_os.py`, routing through the same seams as the read tools so the `DocumentFilter` stays one predicate), `.scratch/` is the only writable path, and a document is written only by committing `/out` through the canonical write path after the program succeeds.
  ...
  A program gets the structured `data` channel as the objects the tool's declared result type serialises to, the same type the stub names and `return_schema` publishes, and the declarations come from pydantic-ai's `FunctionSignature`: once for the model (`declarations`), once as `type_check_stubs` (`stubs`), which `settings.sandbox.type_check` enforces.
