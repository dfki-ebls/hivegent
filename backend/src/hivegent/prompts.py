...
A program calls no tools: `open` and `iterdir` are the read tools, `re` is grep, `json` is jq, a spreadsheet is `query_table` (nothing in here decodes one), and there is no retrieval, so search first and let the program work from the paths it named.
