...
def upgrade() -> None:
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.ForeignKeyConstraint(['compacted_from_id'], ['conversations.id'], name=op.f('fk_conversations_compacted_from_id_conversations'), ondelete='SET NULL'),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_conversations_user_id_users'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_conversations'))
    )
    with op.batch_alter_table('conversations', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_conversations_user_id_updated_at'), ['user_id', 'updated_at'], unique=False)
    ...
    sa.Column('entry_kind', sa.Enum('USER_MARKDOWN', 'IMAGE', 'CONVERTIBLE', 'BINARY_STUB', name='entrykind', native_enum=False), nullable=False),
    sa.Column('origin', sa.Enum('UPLOAD', 'COLLECTION', 'EXTRACTED', name='origin', native_enum=False), nullable=False),
    sa.Column('generated_by', sa.Enum('USER', 'CONVERTER', 'VISION', 'STUB', name='generatedby', native_enum=False), nullable=False),
    sa.Column('mime', sa.String(), nullable=True),
    sa.Column('pipeline', sa.String(), nullable=False),
    sa.Column('content_sha256', sa.String(), nullable=True),
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.CheckConstraint('(owner_user_id IS NOT NULL) <> (owner_group_id IS NOT NULL)', name=op.f('ck_documents_single_owner')),
    sa.ForeignKeyConstraint(['owner_group_id'], ['groups.id'], name=op.f('fk_documents_owner_group_id_groups'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['owner_user_id'], ['users.id'], name=op.f('fk_documents_owner_user_id_users'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_documents')),
    sa.UniqueConstraint('owner_group_id', 'stem_path', name=op.f('uq_documents_owner_group_id_stem_path')),
    sa.UniqueConstraint('owner_user_id', 'stem_path', name=op.f('uq_documents_owner_user_id_stem_path'))
    )
    op.create_table('group_members',
    sa.Column('group_id', sa.String(), nullable=False),
    sa.Column('user_id', sa.String(), nullable=False),
    sa.Column('permission', sa.Enum('READ', 'WRITE', name='permission', native_enum=False), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_memory_user_id_users'), ondelete='CASCADE'),
    ...
    sa.Column('idx', sa.Integer(), nullable=False),
    sa.Column('kind', sa.Enum('REQUEST', 'RESPONSE', name='messagekind', native_enum=False), nullable=False),
